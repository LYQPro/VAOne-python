...
class MainWindow(QMainWindow):
    ...
    def make_layup(self, param):
        ...
        # todo 如何确定参数的合法性 如何在参数不合法的情况下，正确使用缺省参数
        # print(option)
        if option == "soft and hard-layer":
            softHardLayerUp(param['soft_t'])
        elif option == "soft-layer and Eva":
            softEvaLayerUp(param['soft_t'])
            ...
            if param['tsoft_t'] > 0:
                softLayerUP(param['tsoft_t'])
            else:
                softLayerUP()
        else:
            if param['solid_t'] > 0:
                solidLayerUp(param['solid_t'])
            else:
                solidLayerUp()

    def make_mnct(self, param):
        option = self.multiple_trim_option()
        MultipleNoiseControlTreatment('mnct2' + str(round(time.time())), glvm.getv('layeredtrim'), distribution=None)
